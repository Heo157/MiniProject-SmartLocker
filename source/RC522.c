...
void Write_MFRC522(uchar addr, uchar val)
{
    ...
    // MFRC522 스펙에 따라 주소 비트 설정 (쓰기 명령어)
...
uchar Read_MFRC522(uchar addr)
{
    ...
    // MFRC522 스펙에 따라 주소 비트 설정 (읽기 명령어)
    tx_buffer[0] = ((addr << 1) & 0x7E) | 0x80;
    // 데이터를 읽기 위해 더미 바이트를 보냅니다.
    tx_buffer[1] = 0x00;

    /* CS를 LOW로 만듭니다 */
    HAL_GPIO_WritePin(MFRC522_CS_PORT, MFRC522_CS_PIN, GPIO_PIN_RESET);

    // 주소와 더미 바이트를 보내고, 동시에 데이터를 수신합니다.
    HAL_SPI_TransmitReceive(&hspi2, tx_buffer, rx_buffer, 2, 1000);

    /* CS를 HIGH로 되돌립니다 */
    HAL_GPIO_WritePin(MFRC522_CS_PORT, MFRC522_CS_PIN, GPIO_PIN_SET);

    // 수신 버퍼의 두 번째 바이트에 읽어온 데이터가 들어있습니다.
    return rx_buffer[1];
}
